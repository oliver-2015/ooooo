...
// 添加恐龙相关变量
const dinosaurSpeed = moveSpeed * 2; // 恐龙速度是玩家速度的两倍
...
function init() {
    ...
    setInterval(spawnGrenade, 45000);  // 每45秒生成手雷
    setInterval(spawnCoin, 15000);  // 每15秒生成一个金币
    ...
    scene.children.forEach(object => {
        if (object.isFood || object.isMaterial || object.isGrenadeItem || object.isCoin) {
            ...
            if (distance < 1) {
                if (object.isCoin) {
                    score = score + 10;  // 增加分数
                    console.log('获得金币！分数增加10分！当前总分：' + score);
                    scene.remove(object);
                    // 更新UI显示
...
function updateUI() {
    try {
        ...
        document.getElementById('shopScore').textContent = score;  // 更新商城分数显示
...
function throwGrenade() {
    if (!canThrowGrenade || grenadeCount <= 0) return; // 检查是否有手雷可用
    
    grenadeCount--; // 使用一个手雷
    canThrowGrenade = false;
    
    // 创建手雷模型（黑色小球）
    const grenadeGeometry = new THREE.SphereGeometry(0.2); // 稍微增大尺寸
    const grenadeMaterial = new THREE.MeshPhongMaterial({ 
        color: 0x000000,  // 纯黑色
        shininess: 30,    // 添加一些光泽
        emissive: 0x000000 // 添加一些自发光
    });
    ...
    grenade.position.x += rightArm.position.x;
